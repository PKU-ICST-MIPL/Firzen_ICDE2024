...
from itemcoldstart.utils.url import (
    decide_download,
    download_url,
    extract_zip,
    makedirs,
...
class Dataset(torch.utils.data.Dataset):
    ...
    def _data_processing(self):
        ...
        # Obtain item2id mapping file, used to order the img/txt/mm feats
        if self.config['save_item2id_mapping']:
            with open(os.path.join(os.path.dirname(self.config['data_path']), self.config['dataset'], 'item2id.json'), 'w', encoding='utf-8') as fout:
                json.dump(self.field2token_id[self.iid_field], fout)
            print("Successfully save file: item2id.json!")
        # item2id dict
        # e.g., {'[PAD]':0, 'B00LPVG6V0': 249273, 'B00LU0LTOU': 249274}
    ...
    def _multimodal_feat_preparation(self):
        """Sort :attr:`img_feat` and :attr:`txt_feat` by ``item_id``.
        """
        if hasattr(self, "img_feat"):
            if self.img_feat is not None:
                new_item_df = pd.DataFrame({self.iid_field: np.arange(self.item_num)})
                self.v_feat = pd.merge(
                    new_item_df, self.img_feat, on=self.iid_field, how="left"
                )
                self.logger.debug(set_color("ordering img features by item id.", "green"))
        if hasattr(self, "txt_feat"):
            if self.txt_feat is not None:
                self.t_feat = pd.merge(
                    new_item_df, self.txt_feat, on=self.iid_field, how="left"
                )
                self.logger.debug(set_color("ordering txt features by item id.", "green"))
    ...
    def join(self, df):
        ...
        """
        self.item_feat 1683*3
                       item_id     release_year          class
                0           0           0.0                []
                1           1           3.0               [3]
                2           2          39.0          [8, 17, 18, 6]
        """
    ...
    def _grouped_index(self, group_by_list, return_dict = False):
        index = {}
    ...
    def load_splitted_cold_datasets(self):
        """
        Load splitted cold datasets
        Args:
            split_label:
        ...
        # train/ validation
        for i in range(2):
            d = copy.deepcopy(self.inter_feat[self.inter_feat["splitting_label"] == i])
            dfs.append(d)


        # grouped by train set/ valid set respectively
        grouped_inter_feat_index_dict_train = self._grouped_index(
            dfs[0]["item_id"].numpy(),
            return_dict=True
        )
        grouped_inter_feat_index_dict_valid = self._grouped_index(
            dfs[1]["item_id"].numpy(),
            return_dict=True
        )
        grouped_inter_feat_index_dict_all = self._grouped_index(
            self.inter_feat["item_id"].numpy(),
            return_dict=True
        )

        num_item = len(grouped_inter_feat_index_dict_all)
        self.logger.info("num of all items: {}".format(num_item))

        num_item_train = len(grouped_inter_feat_index_dict_train)
        self.logger.info("num of items in train set: {}".format(num_item_train))

        num_item_valid = len(grouped_inter_feat_index_dict_valid)
        self.logger.info("num of items in valid set: {}".format(num_item_valid))

        # hot items
        hot_item_set = set(k for k in grouped_inter_feat_index_dict_train.keys())
        num_hot_items = len(hot_item_set)
        self.logger.info("num of hot items: {}".format(num_hot_items))

        # cold items
        cold_item_set = set(k for k in grouped_inter_feat_index_dict_valid.keys() if k not in grouped_inter_feat_index_dict_train.keys())
        num_cold_items = len(cold_item_set)
        self.logger.info("num of cold items: {}".format(num_cold_items))
        ...
        hot_feat_index = []
        cold_feat_index = []
        for (k, v) in grouped_inter_feat_index_dict_valid.items():
            if k in hot_item_set:
                hot_feat_index.append(list(v))
            else:
                cold_feat_index.append(list(v))

        # hot valid data / hot test data
        hot_index = [[] for _ in range(2)]
        for grouped_index in hot_feat_index:
            tot_cnt = len(grouped_index)
            split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=[0.5,0.5])
            for index, start, end in zip(
                    hot_index, [0] + split_ids, split_ids + [tot_cnt]
            ):
                index.extend(grouped_index[start:end])

        next_df.extend([self.inter_feat[index] for index in hot_index])


        # cold valid data / cold test data
        cold_index = [[] for _ in range(2)]
        for grouped_index in cold_feat_index:
            tot_cnt = len(grouped_index)
            split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=[0.5, 0.5])
            for index, start, end in zip(
                    cold_index, [0] + split_ids, split_ids + [tot_cnt]
            ):
                index.extend(grouped_index[start:end])

        next_df.extend([self.inter_feat[index] for index in cold_index])


        # all valid data / all test data
        next_df.extend([self.inter_feat[index] for index in [hot_index[0]+cold_index[0], hot_index[1]+cold_index[1]]])
        ...
        return next_ds, hot_item_set, cold_item_set

    def split_cold_datasets(self, ratios, cold_ratio=0.2, normal_cold=False):
        """
        Split cold datasets according to pre-defined split label column
        Args:
            split_label:
        ...
        """
        if "splitting_label" in self.inter_feat.columns:
            self.logger.info("Already splitting the dataset, no need to split by ratio again!")
            built_data = self.load_splitted_cold_datasets()
            return built_data

        grouped_inter_feat_index_dict = self._grouped_index(
            self.inter_feat["item_id"].numpy(),
            return_dict=True
        )

        num_item = len(grouped_inter_feat_index_dict)
        self.logger.info("num of items: {}".format(num_item))

        hot_items = int((1 - cold_ratio) * num_item)
        self.logger.info("num of hot items: {}".format(hot_items))
        self.logger.info("num of cold items: {}".format(num_item - hot_items))

        hot_item_set = set()
        cold_item_set = set()
        hot_feat_index = []
        cold_feat_index = []
        for item_cnt, (k, v) in enumerate(grouped_inter_feat_index_dict.items()):
            if item_cnt < hot_items:
                hot_item_set.add(k)
                hot_feat_index.append(list(v))
                #hot_feat_index = grouped_inter_feat_index_dict.values()[0:hot_items]
            else:
                cold_item_set.add(k)
                cold_feat_index.append(list(v))
        #cold_feat_index = grouped_inter_feat_index[hot_items:]

        # train data / hot valid data / hot test data
        hot_index = [[] for _ in range(len(ratios))]
        for grouped_index in hot_feat_index:
            tot_cnt = len(grouped_index)
            split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=ratios)
            for index, start, end in zip(
        ...
        next_df = [self.inter_feat[index] for index in hot_index]

        if normal_cold:
            # normal cold-start item recommendation
            # cold valid known data / cold valid unknown data / cold test known data / cold test known data
            cold_index = [[] for _ in range((len(ratios) - 1)*2)]
            for grouped_index in cold_feat_index:
                tot_cnt = len(grouped_index)
                split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=[0.25, 0.25, 0.25, 0.25])
                for index, start, end in zip(
                    cold_index, [0] + split_ids, split_ids + [tot_cnt]
                ):
                    index.extend(grouped_index[start:end])
            ...
            # all valid known data / all valid unknown data / all test known data / all test unknown data
            next_df.extend([self.inter_feat[index] for index in [cold_index[0], hot_index[1]+cold_index[1], cold_index[2], hot_index[2]+cold_index[3]]])
            
        else:
            # cold valid data / cold test data
            cold_index = [[] for _ in range(len(ratios) - 1)]
            for grouped_index in cold_feat_index:
                tot_cnt = len(grouped_index)
                split_ids = self._calcu_split_ids(tot=tot_cnt, ratios=[0.5, 0.5])
                for index, start, end in zip(
            ...
            # all valid data / all test data
            next_df.extend([self.inter_feat[index] for index in [hot_index[1]+cold_index[0], hot_index[2]+cold_index[1]]])
    ...
    def build(self, normal_cold=False):
        ...
        # used for cold start
            ...
            datasets = self.split_cold_datasets(
                ratios=split_args["COLD"], cold_ratio=0.2, normal_cold=normal_cold
            )
        else:
            raise NotImplementedError(
