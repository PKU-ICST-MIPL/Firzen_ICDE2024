...
from itemcoldstart.data.dataset import Dataset
from itemcoldstart.utils import FeatureSource, FeatureType, set_color
from itemcoldstart.utils.url import decide_download, download_url, extract_zip
...
class KnowledgeBasedDataset(Dataset):
    ...
    def __init__(self, config):
        # Whether to use mmkg or not.
        self.mmkg = config["enable_mmkg"]
    ...
    def _load_kg(self, token, dataset_path):
        ...
        if self.mmkg is True:
            kg_path = os.path.join(dataset_path, f"{token}.mmkg")
        else:
    ...
    def _load_link(self, token, dataset_path):
        ...
        if self.mmkg is True:
            #link_path = os.path.join(dataset_path, f"{token}.mmlink")
            # the difference occurs in .kg and .mmkg, not .link
    ...
    def _merge_item_and_entity(self):
        ...
        #link_num = len(self.item2entity)  # i2e
        link_e2i_num = len(self.entity2item)  # there exists one2many item2entity mapping
        ...
        #print("(((", entity_order[-10:])
        entity_id_map = np.zeros_like(entity_order)
        #for order, i in enumerate(entity_order[1 : link_num + 1]):
        for i in entity_order[1: link_e2i_num + 1]:
            entity_id_map[i] = new_item_token2id[self.entity2item[entity_token[i]]]
        entity_id_map[entity_order[link_e2i_num + 1 :]] = np.arange(
            item_num, item_num + entity_num - link_e2i_num - 1
        )
        new_entity_id2token = np.concatenate(
            [new_item_id2token, entity_token[entity_order[link_e2i_num + 1 :]]]
        )
        for i in range(item_num - link_e2i_num, item_num):
        ...
        #print("**", self.field2token_id[self.entity_field]['mm.20006787'])  #1618762
    ...
    def _create_ckg_sparse_matrix(self, form="coo", show_relation=False, add_normal_entity=True):
        ...
        if add_normal_entity:
            ...
            src = np.concatenate([uids, iids])
            tgt = np.concatenate([iids, uids])
            if not show_relation:
                data = np.ones(len(src))
            else:
                ui_rel = np.full(2 * ui_rel_num, ui_rel_id)
                data = ui_rel
    ...
    def _create_ckg_graph(self, form="dgl", show_relation=False):
        ...
        #print(head_entity[-10:])  392577, 391427, 397500, 367168, 394341, 355796, 380994, 338230, 387380, 336634
        #print(tail_entity[-10:])  2049473, 2049474, 2049475, 2049476, 2049477, 2049478, 2049479, 2049480, 2049481, 2049482
    ...
    def ckg_graph(self, form="coo", value_field=None, add_normal_entity=True):
        ...
        if form in ["coo", "csr"]:
            return self._create_ckg_sparse_matrix(form, show_relation, add_normal_entity=add_normal_entity)
        elif form in ["dgl", "pyg"]:
