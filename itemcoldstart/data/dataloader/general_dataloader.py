"""
itemcoldstart.data.dataloader.general_dataloader
################################################
"""
...
from itemcoldstart.data.dataloader.abstract_dataloader import (
    AbstractDataLoader,
    NegSampleDataLoader,
)
from itemcoldstart.data.interaction import Interaction, cat_interactions
...
class TrainDataLoader(NegSampleDataLoader):
    ...
    def collate_fn(self, index):
        ...
        if self.sample_by == "item":
            return self._neg_sampling_user(transformed_data)
        else:
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        for uid, iid in zip(
            ...
            if uid != last_uid:
                ...
                self.uid_list.append(uid)
                positive_item = set()
            # other interactions of the same user
