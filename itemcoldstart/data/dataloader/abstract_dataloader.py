"""
itemcoldstart.data.dataloader.abstract_dataloader
################################################
"""
...
from itemcoldstart.utils import InputType, FeatureType, FeatureSource, ModelType
...
class NegSampleDataLoader(AbstractDataLoader):
    ...
    def _set_neg_sample_args(self, config, dataset, dl_format, neg_sample_args):
        self.sample_by = neg_sample_args["sample_by"]
        ...
        if (
            ...
            if self.dl_format == InputType.POINTWISE:
                ...
                if self.sample_by == 'item':
                    self.sampling_func = self._neg_sample_by_point_wise_sampling_user_group_by_user
                else:
                ...
                if self.sample_by == 'item':
                    self.sampling_func = self._neg_sample_by_pair_wise_sampling_group_by_user

                    self.neg_prefix = config["NEG_PREFIX"]
                    self.neg_user_id = self.neg_prefix + self.uid_field

                    columns = (
                        [self.uid_field]
                        if dataset.user_feat is None
                        else dataset.user_feat.columns
                    )
                    for user_feat_col in columns:
                        neg_user_feat_col = self.neg_prefix + user_feat_col
                        dataset.copy_field_property(neg_user_feat_col, user_feat_col)
                else:
    ...
    def _neg_sampling_user(self, inter_feat):
        if self.neg_sample_args.get("dynamic", False):
            ...
            neg_candidate_ids = self._sampler.sampler_by_item_ids(
            ...
            neg_user_feat = Interaction(
                {self.uid_field: neg_candidate_ids.to(self.model.device)}
            )
            interaction.update(neg_user_feat)
            ...
            neg_user_ids = neg_candidate_ids[
                indices, [i for i in range(neg_candidate_ids.shape[1])]
            ].view(-1)
            self.model.train()
            return self.sampling_func(inter_feat, neg_user_ids)
        elif (
            self.neg_sample_args["distribution"] != "none"
            and self.neg_sample_args["sample_num"] != "none"
        ):
            user_ids = inter_feat[self.uid_field].numpy()
            item_ids = inter_feat[self.iid_field].numpy()
            neg_user_ids = self._sampler.sampler_by_item_ids(
    ...
    def _neg_sample_by_pair_wise_sampling_group_by_user(self, inter_feat, neg_user_ids):
        inter_feat = inter_feat.repeat(self.times)
        ...
        inter_feat.update(neg_user_feat)
        return inter_feat
    ...
    def _neg_sample_by_point_wise_sampling_user_group_by_user(self, inter_feat, neg_user_ids):
        pos_inter_num = len(inter_feat)
        new_data = inter_feat.repeat(self.times)
        new_data[self.uid_field][pos_inter_num:] = neg_user_ids
        new_data = self._dataset.join(new_data)
        labels = torch.zeros(pos_inter_num * self.times)
        labels[:pos_inter_num] = 1.0
        new_data.update(Interaction({self.label_field: labels}))
