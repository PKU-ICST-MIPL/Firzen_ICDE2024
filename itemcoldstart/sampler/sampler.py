...
class AbstractSampler(object):
    ...
    def __init__(self, distribution, alpha, sample_by='user'):
        self.distribution = ""
        self.alpha = alpha
        self.set_distribution(distribution)
        if sample_by == 'item':
            self.used_ids = self.get_used_user_ids()
        else:
    ...
    def sampling(self, sample_num):
        ...
        """
        if self.sample_by == 'item':
            if self.distribution == "uniform":
                return self._uni_sampling_user(sample_num)
            elif self.distribution == "popularity":
                return self._pop_sampling_user(sample_num)
            else:
    ...
    def get_used_user_ids(self):
        """
        Returns:
            numpy.ndarray: Used user ids. Index is key_id, and element is a set of value_ids.
        """
        raise NotImplementedError("Method [get_used_user_ids] should be implemented")
    ...
    def sample_by_key_ids(self, key_ids, num):
        ...
        if (key_ids == key_ids[0]).all():
            ...
            # check whether the sampled items in used_ids
...
class Sampler(AbstractSampler):
    ...
    def __init__(self, phases, datasets, distribution="uniform", alpha=1.0, sample_by="user"):
        ...
        # Here user_num and item_num should be the total num instead of num in training set
        self.user_num = datasets[0].user_num
        self.item_num = datasets[0].item_num

        super().__init__(distribution=distribution, alpha=alpha, sample_by=sample_by)
    ...
    def _uni_sampling_user(self, sample_num):
        return np.random.randint(1, self.user_num, sample_num)
    ...
    def get_used_ids(self):
        ...
        for phase, dataset in zip(self.phases, self.datasets):
            ...
            for uid, iid in zip(
                ...
                cur[uid].add(iid)
            # used_items for eval: positive items in train sets
            # used_items for test: positive items in train and eval sets
    ...
    def get_used_user_ids(self):
        """
        Returns:
            dict: Used user_ids is the same as positive user_ids.
            Key is phase, and value is a numpy.ndarray which index is item_id, and element is a set of user_ids.
        """

        used_user_id = dict()

        last = [set() for _ in range(self.item_num)]
        for phase, dataset in zip(self.phases, self.datasets):
            cur = np.array([set(s) for s in last])
            for uid, iid in zip(
                dataset.inter_feat[self.uid_field].numpy(),
            ...
            last = used_user_id[phase] = cur

        for used_user_set in used_user_id[self.phases[-1]]:
            if len(used_user_set) + 1 == self.user_num:  # [pad] is a item.
                raise ValueError(
                    "Some items have interacted with all users, "
                    "which we can not sample negative users for them. "
                    "Please set `item_inter_num_interval` to filter those items."
                )
        return used_user_id
    ...
    def sample_multiple_positives(self, key_ids, num):
        """Sampling by key_ids.
        ...
        if (key_ids == key_ids[0]).all():
            ...
            value_ids = self.sampling(total_num)
            # check whether the sampled items in used_ids
            check_list = np.arange(total_num)[not np.isin(value_ids, used)]
            while len(check_list) > 0:
                value_ids[check_list] = value = self.sampling(len(check_list))
                mask = np.isin(value, used)
                check_list = check_list[mask]
    ...
    def sample_multiple_negatives(self, key_ids, pos_num, neg_num):
        """Sampling by key_ids.
        ...
        key_ids = np.array(key_ids)
        key_num = len(key_ids)
        total_num = key_num * neg_num * pos_num
        if (key_ids == key_ids[0]).all():
            key_id = key_ids[0]
            used = np.array(list(self.used_ids[key_id]))
            value_ids = self.sampling(total_num)
            ...
            value_ids = np.zeros(total_num, dtype=np.int64)
            check_list = np.arange(total_num)
            key_ids = np.tile(key_ids, neg_num * pos_num)
            while len(check_list) > 0:
                value_ids[check_list] = self.sampling(len(check_list))
                check_list = np.array(
                    [
    ...
    def sample_multiple_positives_for_user_ids(self, user_ids, item_ids, num):
        """Sampling positive items for users."""
        try:
            return self.sample_multiple_positives(user_ids, num)
        except IndexError:
            for user_id in user_ids:
                if user_id < 0 or user_id >= self.user_num:
                    raise ValueError(f"user_id [{user_id}] not exist.")

    def sample_by_item_ids(self, user_ids, item_ids, num):
        """Sampling by item_ids

        Args:
            user_ids: (numpy.ndarray or list): Input user_ids.
            item_ids: (numpy.ndarray or list): Input item_ids.
            num (int): Number of sampled user_ids for each item_id.

        Returns:
            torch.tensor: Sampled user_ids.
        """
        try:
            return self.sample_by_key_ids(item_ids, num)
        except IndexError:
            for item_id in item_ids:
                if item_id < 0 or item_id >= self.item_num:
                    raise ValueError(f"item_id [{item_id}] not exist.")
