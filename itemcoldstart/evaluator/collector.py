"""
itemcoldstart.evaluator.collector
################################################
"""

from itemcoldstart.evaluator.register import Register
import torch
import copy
...
class Collector(object):
    ...
    def data_collect(self, train_data):
        ...
        if self.register.need("data.count_users"):
            self.data_struct.set("data.count_users",
    ...
    def eval_batch_collect(
        ...
        if self.register.need("rec.meanrank"):
            desc_scores, desc_index = torch.sort(
                scores_tensor, dim=-1, descending=True)
            ...
            result = torch.cat(
                (pos_rank_sum, user_len_list, pos_len_list), dim=1)
