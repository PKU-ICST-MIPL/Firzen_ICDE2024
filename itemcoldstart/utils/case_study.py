"""
itemcoldstart.utils.case_study
#####################################
"""
...
def full_sort_scores(uid_series, model, test_data, device=None):
    ...
    try:
        ...
        input_interaction = input_interaction.repeat_interleave(
            dataset.item_num)
...
@torch.no_grad()
def full_sort_scores_cold(uid_series, model, test_data, extra_mask_index=None, device=None):
    """Calculate the scores of all items for each user in uid_series.
    ...
    if history_index is not None:
        scores[history_index] = -np.inf  # set scores of history items to -inf
    if extra_mask_index is not None:
        scores[extra_mask_index] = -np.inf
...
def full_sort_topk_cold(uid_series, model, test_data, k, extra_mask_index, device=None):
    """Calculate the top-k items' scores and ids for each user in uid_series.
    ...
    Returns:
        tuple:
            ...
            - topk_index (torch.Tensor): The index of topk items, which is also the internal ids of items.
    """
    scores = full_sort_scores_cold(
        uid_series, model, test_data, extra_mask_index, device)
    return torch.topk(scores, k)
