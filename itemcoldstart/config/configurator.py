"""
itemcoldstart.config.configurator
################################
"""
...
from itemcoldstart.evaluator import metric_types, smaller_metrics
...
class Config(object):
    ...
    def _load_internal_config_dict(self, model, model_class, dataset):
        ...
        if self.internal_config_dict["MODEL_TYPE"] == ModelType.KNOWLEDGE:
            self._update_internal_config_dict(knowledge_base_init)
        else:
            raise NotImplementedError("The model type of Firzen should be KNOWLEDGE.")
